...
function normalizeWeaviateUrl(url) {
...
async function fetchWeaviate(query, url, apiKey) {
...
async function fetchAllEvidenceByIds(ids, url, apiKey) {
  const uniqueIds = [
    ...new Set(ids)
  ];
  ...
  return uniqueIds.map((id)=>evidenceById.get(id)).filter((item)=>item !== undefined);
}
serve(async (req)=>{
