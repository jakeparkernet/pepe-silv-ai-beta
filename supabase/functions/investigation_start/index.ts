...
function normalizeQueueUrlToRawUrl(storedQueueUrl) {
...
function normalizePrefetched(value) {
  ...
  const siteId = typeof value.site_id === "string" ? value.site_id : typeof siteData?.site_id === "string" ? siteData.site_id : typeof siteData?.site_record_id === "string" ? siteData.site_record_id : "";
  const siteEntityId = typeof value.site_entity_id === "string" ? value.site_entity_id : typeof siteData?.site_entity_id === "string" ? siteData.site_entity_id : typeof siteData?.news_site_entity_id === "string" ? siteData.news_site_entity_id : null;
  const siteNewsSite = typeof value.site_news_site === "string" ? value.site_news_site : typeof siteData?.news_site === "string" ? siteData.news_site : null;
  ...
  const extractedCompanies = Array.isArray(value.extracted_companies) ? value.extracted_companies.filter(isRecord).map((company)=>({
  ...
  const applicabilityResult = isRecord(value.applicability_result) ? {
    ...
    reason: typeof value.applicability_result.reason === "string" ? value.applicability_result.reason : "",
    identified_company: typeof value.applicability_result.identified_company === "string" ? value.applicability_result.identified_company : null,
    identified_product: typeof value.applicability_result.identified_product === "string" ? value.applicability_result.identified_product : null
  } : undefined;
  const articleSubjectEntity = isRecord(value.article_subject_entity) ? value.article_subject_entity : null;
  if (!domain || !siteId) {
    return undefined;
  }
  return {
...
function buildEnqueuePayload(rawUrl, prefetched) {
...
function flyHeaders(flyToken) {
  return {
    Authorization: `Bearer ${flyToken}`,
    "Content-Type": "application/json"
  };
}
function flyHeadersWithLease(flyToken, leaseNonce) {
  return {
    Authorization: `Bearer ${flyToken}`,
    "Content-Type": "application/json",
    ...leaseNonce ? {
...
function coordinatorHeaders(machineId, apiKey) {
  return {
    ...apiKey ? {
...
async function listFlyMachines(flyApiHostname, flyToken, appName) {
...
async function leaseFlyMachine(flyApiHostname, flyToken, appName, machineId, ttlSeconds) {
...
async function releaseFlyMachineLease(flyApiHostname, flyToken, appName, machineId, leaseNonce) {
  const url = `${flyApiHostname}/v1/apps/${appName}/machines/${machineId}/lease`;
  return await safeFetchJson(url, {
    method: "DELETE",
...
async function startFlyMachine(flyApiHostname, flyToken, appName, machineId, leaseNonce) {
...
function parseLeaseNonce(bodyJson) {
  if (!isRecord(bodyJson)) {
    return null;
  }
  const directNonce = typeof bodyJson.nonce === "string" && bodyJson.nonce.length > 0 ? bodyJson.nonce : null;
  if (directNonce) return directNonce;
  const data = isRecord(bodyJson.data) ? bodyJson.data : null;
  if (!data) return null;
  const nestedNonce = typeof data.nonce === "string" && data.nonce.length > 0 ? data.nonce : null;
  return nestedNonce;
}
...
function sortMachinesOldestUpdateFirst(machines) {
  return [
    ...machines
  ].sort((a, b)=>{
...
function parsePositiveIntegerSetting(value) {
...
async function getFlyScaleLimit(supabase, requestId) {
  logInfo(requestId, "db.settings.fly_scale_lookup.start");
  const settingsRes = await supabase.from("settings").select("key, value").eq("key", "fly_scale").maybeSingle();
...
async function clearRemoteRequestedAt(args) {
  ...
  const resetRes = await supabase.from("article_queue").update({
    remote_requested_at: null
  }).eq("id", queueId).select("id, remote_requested_at, status").maybeSingle();
  if (resetRes.error) {
...
async function markDispatchSlotDispatched(args) {
  const { supabase, reservationId, machineId, enqueuePayload, ttlSeconds = 900 } = args;
...
async function claimPooledMachine(args) {
  ...
  const machines = listRes.bodyJson;
  const candidates = sortMachinesOldestUpdateFirst(machines.filter((machine)=>isMachineAvailable(machine)));
  ...
  for (const candidate of candidates){
    const machineId = typeof candidate.id === "string" && candidate.id.length > 0 ? candidate.id : null;
    const machineState = getMachineState(candidate);
    if (!machineId || machineState !== "stopped") continue;
    const leaseRes = await leaseFlyMachine(flyApiHostname, flyToken, appName, machineId, 60);
...
async function startClaimedMachine(args) {
  ...
  const startRes = await startFlyMachine(flyApiHostname, flyToken, appName, machineId, leaseNonce);
  if (!startRes.ok) {
    return {
      ok: false,
      reason: "start_failed",
...
serve(async (req)=>{
  ...
  try {
    ...
    let queueLookup;
    if (requestQueueId) {
      queueLookup = await supabase.from("article_queue").select("*").eq("id", requestQueueId).maybeSingle();
      ...
      queueLookup = await supabase.from("article_queue").select("*").eq("url", queueUrlKey).maybeSingle();
    } else {
      return jsonResponse(400, {
    ...
    const queueRow = queueLookup.data;
    ...
    let machineIdForCleanup = null;
    let leaseNonceForCleanup = null;
    ...
    if (!slotRes.ok || !slotRes.reservation_id) {
      ...
      const markRes = await supabase.from("article_queue").update({
        remote_requested_at: new Date().toISOString()
      }).eq("id", resolvedQueueId).in("status", [
        "queued",
        "deferred"
      ]).is("remote_requested_at", null).select("id").maybeSingle();
      if (markRes.error) {
        ...
        try {
          ...
          if (!didCallRemote) {
            await clearRemoteRequestedAt({
              ...
              queueId: resolvedQueueId,
              requestId,
              reason: dispatchResult && typeof dispatchResult.reason === "string" ? dispatchResult.reason : "remote_not_dispatched"
            });
            if (reservationId) {
              await releaseDispatchSlot({
                supabase,
    ...
    const finalQueueRes = await supabase.from("article_queue").select("*").eq("id", resolvedQueueId).maybeSingle();
    return jsonResponse(200, {
