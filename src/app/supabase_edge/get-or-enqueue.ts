...
const ALLOWED_ORIGINS = new Set(
  (Deno.env.get("ALLOWED_ORIGINS") ?? "http://localhost:3000,http://localhost:5173,http://127.0.0.1:3000,http://127.0.0.1:5173,https://pepesilv.ai,https://www.pepesilv.ai")
    .split(",")
    .map((o) => o.trim())
    .filter(Boolean)
);
...
function getSiteDomain(hostname: string): string {
...
const SCRAPE_LAMBDA_ARN =
  Deno.env.get("SCRAPE_PAGE_ARN") ||
  "scrape_page";
const LLM_LAMBDA_ARN =
  Deno.env.get("GET_LLM_RESPONSE_ARN") ||
  "get_llm_response";
...
function getWeaviateConfig(): { url: string; apiKey: string } | null {
  let url = (Deno.env.get("WEAVIATE_URL") ?? "").trim();
  const apiKey =
    Deno.env.get("WEAVIATE_API_KEY") ??
    Deno.env.get("WEAVIATE_APIKEY") ??
    "";
...
async function invokeFunctionUrl<T>(url: string, payload: Record<string, unknown>): Promise<T> {
  const headers: Record<string, string> = {
  ...
  if (
    ...
    const bodyText = (parsed as { body: string }).body;
...
type NewsSiteEntityResolution = {
  newsSiteId: string | null;
  newsSiteDomain: string | null;
...
function getFirstWord(text: string | null | undefined): string | null {
  if (text == null) return null;
  return text.length > 0 ? text.split(/\s+/, 1)[0] : text;
}

function getSearchableNamePrefix(searchName: string, maxChars = 0): string {
...
async function invokeGenericLlm(
  ...
  if (GET_LLM_RESPONSE_URL) {
    const res = await invokeFunctionUrl<{
      status_code: number;
      result: string;
      full_completion?: Record<string, unknown>;
    }>(GET_LLM_RESPONSE_URL, payload);
...
}): Promise<{ same_entity: boolean; confidence: number }> {
  const systemMessage = `
...
async function selectMostLikelyEntity(args: {
  ...
  const parsed = parseJsonText(raw) as {
    ...
    result?: { id?: string; name?: string; confidence?: number } | null;
  };
  ...
  const sorted = resultEntities
    .filter((entity) => typeof entity.id === "string" && typeof entity.confidence === "number")
...
async function queryWeaviateEntities(query: string): Promise<WeaviateQueryResult> {
  ...
  if (!weaviateConfig) {
    return {
      ...
      ok: false,
      status: null,
      errors: [{ message: "Missing Weaviate configuration" }],
    };
  }
  ...
  if (!response.ok) {
    return {
      ...
      errors: [{ message: "Non-OK Weaviate response" }],
    };
  }

  const json = await response.json();
  const entities = (json?.data?.Get?.Entity || []) as Array<Record<string, unknown>>;
    ...
    entities: entities
      .map((entity) => ({
        ...
        tags: Array.isArray(entity.tags)
          ? entity.tags.filter((tag): tag is string => typeof tag === "string")
          : [],
        context: typeof entity.context === "string" ? entity.context : "",
        flatname: typeof entity.flatname === "string"
          ? entity.flatname
          : normalizeLettersOnly(typeof entity.name === "string" ? entity.name : ""),
      }))
      .filter((entity) => entity.id.length > 0 && entity.name.length > 0),
    ok: true,
    status: response.status,
    errors: Array.isArray(json?.errors) ? json.errors as unknown[] : null,
  };
}

async function queryWeaviateJson(query: string): Promise<{
  ok: boolean;
  status: number | null;
...
async function resolveNewsSiteEntity(args: {
  ...
  domain: string;
  requestId?: string;
}): Promise<NewsSiteEntityResolution> {
  const queries = [
    `{
      Get {
        NewsSite(where: {
  ...
  for (const [index, query] of queries.entries()) {
    ...
    const rows = (result.data?.Get as { NewsSite?: Array<Record<string, unknown>> } | undefined)?.NewsSite ?? [];
    if (args.requestId) {
    ...
    const firstEntity = entityRefs[0] as { uuid?: string; name?: string } | undefined;
...
async function getEntitiesByNameWeaviate(name: string): Promise<EntityCandidate | null> {
  const queries = [
    `{
      Get {
        Entity(where: {
...
async function getEntitiesWithAliasWeaviate(alias: string): Promise<EntityCandidate[]> {
  const query = `{
    Get {
      Entity(where: {
        path: ["aliases"]
        operator: ContainsAny
        valueText: ${JSON.stringify([alias])}
      }) {
        uuid
        name
        aliases
...
async function getEntitiesLikeWeaviate(name: string): Promise<EntityCandidate[]> {
  const query = `{
    Get {
      Entity(where: {
        path: ["name"]
...
async function getEntitiesContainsWeaviate(name: string): Promise<EntityCandidate[]> {
  const query = `{
    Get {
      Entity(where: {
        path: ["name"]
...
async function getEntitiesStartsWithWeaviate(prefix: string): Promise<EntityCandidate[]> {
  const query = `{
    Get {
      Entity(where: {
        path: ["name"]
...
async function getEntitiesNearTextWeaviate(text: string): Promise<EntityCandidate[]> {
  const query = `{
    Get {
      Entity(
        nearText: {
...
function dedupeEntityCandidates(entities: EntityCandidate[]): EntityCandidate[] {
...
): Promise<{ id: string; name: string; weaviate_uuid?: string }> {
  const weaviateConfig = getWeaviateConfig();
...
async function findExistingOwnershipTree(
  companyAId: string,
  companyBId: string,
  supabaseUrl: string,
  supabaseServiceKey: string,
): Promise<OwnershipTreeRow | null> {
  const supabase = createClient(supabaseUrl, supabaseServiceKey);
  const scanResponse = await supabase
  ...
  let matchedTreeId: string | null = null;
  for (const row of scanResponse.data as Array<Pick<OwnershipTreeRow, "id" | "company_a" | "company_b">>) {
    const a = row.company_a;
    const b = row.company_b;
    if (
      (a === companyAId && b === companyBId) ||
      (a === companyBId && b === companyAId)
    ) {
  ...
  const matchedResponse = await supabase
    .from("ownership_trees")
  ...
  return matchedResponse.data as OwnershipTreeRow;
}

async function invokeScrapeLambda(url: string): Promise<{ status_code: number; result: { raw_html: string; result: string } }> {
  if (SCRAPE_PAGE_URL) {
...
async function invokeExtractEntityLambda(
  articleText: string,
  articleTitle: string,
): Promise<{ status_code: number; result: string; full_completion?: Record<string, unknown> }> {
...
async function invokeApplicabilityLambda(
  articleTitle: string,
  articleText: string,
): Promise<{ status_code: number; result: string; full_completion?: Record<string, unknown> }> {
...
async function invokeSelectPrimaryCompanyLambda(
  articleTitle: string,
...
function serializePrefetchedEntity(
  entity: { id: string; name: string },
  options: { entityType?: string; tags?: string[]; context?: string } = {},
): PrefetchedEntity {
  return {
    id: entity.id,
    created_at: "None",
...
async function buildApplicabilityResult(
  articleText: string,
  articleTitle: string,
        ...
        return {
          ...
          reason:
            typeof parsed.reason === "string" && parsed.reason.trim().length > 0
              ? parsed.reason.trim()
              : parsed.is_applicable === true
                ? "Article is applicable for ownership investigation."
                : "Article is not applicable for ownership investigation.",
          identified_company:
            typeof parsed.identified_company === "string" && parsed.identified_company.trim().length > 0
              ? parsed.identified_company.trim()
              : null,
          identified_product:
            typeof parsed.identified_product === "string" && parsed.identified_product.trim().length > 0
              ? parsed.identified_product.trim()
              : null,
        };
      }
    }
...
async function selectPrimaryCompanyResult(
  articleTitle: string,
  companies: ExtractedCompany[],
): Promise<ApplicabilityResult> {
  ...
  if (companies.length === 1) {
    return {
      is_applicable: true,
      reason: companies[0].context
        ? `Single company identified: ${companies[0].context}`
        : "Single company identified in article",
  ...
  try {
    ...
    if (llmRaw.status_code === 200) {
      const content = typeof llmRaw.result === "string" ? llmRaw.result : null;
      if (content) {
        const parsed = JSON.parse(content) as { selected_company?: string; reason?: string };
        const selectedCompany = typeof parsed.selected_company === "string" ? parsed.selected_company : null;
        const matchingCompany = selectedCompany
        ...
        if (matchingCompany) {
          return {
            ...
            reason: typeof parsed.reason === "string" && parsed.reason.length > 0
              ? parsed.reason
              : "Selected as primary company from multiple candidates",
...
async function runEdgePreInvestigation(args: {
  ...
  try {
    ...
    const resolvedCompany =
      applicabilityResult.identified_company && applicabilityResult.identified_company.length > 0
        ? applicabilityResult.identified_company
        : subjectSelectionResult.identified_company;
    ...
    const resolvedApplicabilityResult: ApplicabilityResult = {
      ...applicabilityResult,
    ...
    if (newsSiteEntityId) {
      ...
      const existingOwnershipTree = await findExistingOwnershipTree(
        newsSiteEntityId,
        articleSubject.id,
        supabaseUrl,
        supabaseServiceRole,
      ...
      if (DEBUG_THROW_IF_OWNERSHIP_TREE_MISSING) {
        throw new Error(
          `DEBUG ownership tree miss after subject resolution: news_site_entity_id=${newsSiteEntityId}, article_subject_id=${articleSubject.id}, article_subject_name=${articleSubject.name}`,
        );
      }
    }
...
async function callInvestigationStartEdge(args: {
  ...
  supabaseUrl: string;
  supabaseServiceRole: string;
  internalEdgeApiKey: string;
}): Promise<{
  ok: boolean;
  error?: string;
  did_call_remote?: boolean;
  fly_scale?: number;
  ...
  return {
    ...
    dispatch_result: typeof body.dispatch_result === "object"
      ? body.dispatch_result as Record<string, unknown>
      : null,
    queue: typeof body.queue === "object"
      ? body.queue as Record<string, unknown>
      : null,
  };
}
...
serve(async (req: Request) => {
  ...
  const respond = (
    ...
    body: Json,
    extraHeaders: Record<string, string> = {},
  ) => jsonResponse(status, body, extraHeaders, origin);

  // Quick early reject for non-allowed origins (extra layer)
  if (origin && !ALLOWED_ORIGINS.has(origin)) {
    logInfo(requestId, "security.cors_blocked", { origin });
  ...
  try {
    ...
    if (req.method === "OPTIONS") {
      const corsHeaders = getCorsHeaders(origin);
      logInfo(requestId, "request.options", { allowed: !!corsHeaders["Access-Control-Allow-Origin"] });
    ...
    if (req.method !== "POST") {
      logInfo(requestId, "request.invalid_method", { method: req.method });
      return respond(405, {
    ...
    const useEdgePreInvestigation = (body as { use_edge_pre_investigation_check?: unknown })?.use_edge_pre_investigation_check === true;
    const deferred = (body as { defer_investigation?: unknown })?.defer_investigation === true;
    ...
    if (!normalizedUrl) {
      logInfo(requestId, "request.invalid_url", { url });
      return respond(400, {
    ...
    const siteId = siteLookup.data.id as string;
    ...
    const initialQueueStatus = useEdgePreInvestigation
      ? "prepass: scraping article"
      : "queued";
    ...
    if (insertRes.error) {
      ...
      const existingQueueRes = await supabase
        .from("article_queue")
        .select("*")
        .eq("url", queueUrlKey)
        .maybeSingle();
      ...
      queueRow = existingQueueRes.data as Record<string, unknown> | null;
    } else {
      queueRow = insertRes.data as Record<string, unknown>;
      wasInserted = true;
    ...
    const queueId =
      queueRow && typeof queueRow.id === "string" && queueRow.id.length > 0
        ? queueRow.id
        : null;
    ...
    let edge_pre_result: unknown = null;
    let investigation_start_result: Record<string, unknown> | null = null;
    ...
    if (useEdgePreInvestigation) {
      ...
      const edgeRes = await runEdgePreInvestigation({
        ...
        site: {
          ...
          news_site:
            typeof siteLookup.data.news_site === "string"
              ? siteLookup.data.news_site
              : null,
        },
        requestId,
        supabase,
      ...
      if (!edgeRes.ok) {
        ...
        const failedQueueRes = await supabase
          .from("article_queue")
          .select("*")
          .eq("url", queueUrlKey)
          .maybeSingle();
      ...
      if (edgeRes.ok && edgeRes.domain && edgeRes.site_id) {
        if (edgeRes.skip_fly && edgeRes.final_output_obj) {
          ...
          const completedQueueRes = await supabase
            .from("article_queue")
            .select("*")
            .eq("url", queueUrlKey)
            .maybeSingle();
        ...
        prefetched = {
          ...
          scrape_result: edgeRes.scrape_result ?? { raw_html: "", result: "" },
        ...
        const prepassSaveRes = await supabase
          .from("article_queue")
          .update({
            investigation_prepass_results: JSON.stringify(investigationPrepassResults),
        ...
        if (edgeRes.not_applicable && edgeRes.applicability_result) {
          ...
          const notApplicableQueueRes = await supabase
            .from("article_queue")
            .select("*")
            .eq("url", queueUrlKey)
            .maybeSingle();
        ...
        if (deferred && prefetched.extracted_companies.length > 0) {
          const articleSubjectName =
            prefetched.applicability_result?.identified_company ??
            prefetched.extracted_companies[0].name;
          const articleSubjectId =
            prefetched.article_subject_entity?.id ??
            null;
          ...
          const deferredQueueRes = await supabase
            .from("article_queue")
            .select("*")
            .eq("url", queueUrlKey)
            .maybeSingle();
    ...
    if (queueId !== null) {
      const currentQueueRes = await supabase
        .from("article_queue")
        .select("status, remote_requested_at")
        .eq("id", queueId)
        .maybeSingle();

      if (!currentQueueRes.error && currentQueueRes.data) {
        currentQueueStatus =
          typeof currentQueueRes.data.status === "string"
            ? currentQueueRes.data.status
            : currentQueueStatus;
