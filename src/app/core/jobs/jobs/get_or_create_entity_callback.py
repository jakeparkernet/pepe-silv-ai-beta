...
@Job.register(name="get_or_create_entity_callback")
class GetOrCreateEntityCallback(Job):
    ...
    label: str = "Get Most Likely Entity"
    description: str = "Finds the most likely entity in a list from Weaviate. Filters using an LLM if necessary."

    async def run(self, platform: str):
        await super().run(platform)

        validate_like_with_llm = get_value_safe(self.input, "validate_with_llm", False)
        ...
        if entity is not None:
            self._set_output({"result": entity.id})
            self.complete()
            return
        ...
        if len(entities) == 1:
            ...
            if validate_like_with_llm:
                ...
                async def on_compare (compare_job):
                    confidence = compare_job.output["confidence"]
    ...
    async def run_deep_search (self):
        check_all_entities_with_llm = get_value_safe(self.input, "check_all_entities_with_llm", False)
        ...
        if check_all_entities_with_llm == False:
            ...
            self._set_output({"result": entity_id})
            self.complete()
            return
        
        if entities is None or len(entities) == 0:
            if near_text is not None:
                entities = await service.get_entities_near_text(near_text)
            else:
                entities = await service.get_entities_near_text(name)
